...
speditor *TheSpeditor = new speditor();
    ...
    case WM_INITDIALOG:
        {
			...
			if(new_add_d == 1)
			{
				SendMessage(c5a, CB_SETCURSEL, TheSpeditor->id, 0);
			}
			else
			{
...
void PrintDoc(LPCTSTR did, LPCTSTR pod, LPCTSTR pdo, LPCTSTR dng, LPCTSTR plc)
{
int r = 0;
int PrintStatus = 1;
int idng = atoi(dng);
...
s_fax_size = strlen(TheSpeditor->fax);
...
strcpy(s_fax, TheSpeditor->fax);
strcpy(s_driver, TheDriver->name);
strcpy(s_dribir, TheDriver->birth);
strcpy(s_drijbs, TheDriver->jobstart);
strcpy(s_dridoc, TheDriver->nrdoc);
       ...
       if(hResult == S_OK && pPDX->dwResultAction == 1)
          {
               ...
               if(r != SP_ERROR)
               {
                   ...
                   TextOut(pPDX->hDC, 1095, -520-(line_h*2), s_line, s_line_size);
                   ...
                   TextOut(pPDX->hDC, 1210, -520-(line_h*3), TheSpeditor->phone, strlen(TheSpeditor->phone));
                   ...
                   TextOut(pPDX->hDC, 1170, -520-(line_h*4), s_fax, s_fax_size);
                   TextOut(pPDX->hDC, 220, -520-(line_h*5), "(5)", 3);
                   TextOut(pPDX->hDC, 370, -520-(line_h*5), "Adres email:", 12);
                   TextOut(pPDX->hDC, 560, -520-(line_h*5), TheSpeditor->email, strlen(TheSpeditor->email));
                   SelectObject(pPDX->hDC, TNR9_B);
                   ...
                   TextOut(pPDX->hDC, 1160, -520-(line_h*12), s_dridoc, strlen(s_dridoc));
                   ...
                   TextOut(pPDX->hDC, 1285, -520-(line_h*13), s_drijbs, strlen(s_drijbs));
                   SelectObject(pPDX->hDC, TNR9_B);
...

{
    ...
    hwnd = CreateWindowEx (
     ...
     t0a = CreateWindow("STATIC","URLOPÓWKI v.1", WS_CHILD | WS_VISIBLE, 170, 5, 250, 25, hwnd, (HMENU)110, NULL, hwnd);  
     ...
     t5 = CreateWindow("STATIC","Czynnoœæ: ", WS_CHILD | WS_VISIBLE | SS_RIGHT, 15, 177, 80, 25, hwnd, (HMENU)14, NULL, hwnd);
     ...
     c55 = CreateWindowEx( 0, "BUTTON", "Wykonywa³ pracê inn¹ ni¿ prowadzenie pojazdu", WS_CHILD | WS_VISIBLE | BS_AUTORADIOBUTTON, 110, 300, 350, 30, hwnd, (HMENU)55, NULL, NULL );
     c56 = CreateWindowEx( 0, "BUTTON", "Pozostawa³ w gotowoœci", WS_CHILD | WS_VISIBLE | BS_AUTORADIOBUTTON, 110, 330, 300, 30, hwnd, (HMENU)56, NULL, NULL );
...
{    
    ...
    LPTSTR val5 = (LPTSTR) "v5"  ; //czynnoœæ
    ...
    case WM_COMMAND: //obsluga klikniêcia w kontrolkê
          ...
          {            
                ...
                case 7: //Button save&print
                     ...
                     if(TLN == 0 || TheDriver->n_id == 0)
                     {
                     FMessage("Wpisz miasto i/lub kierowcê!");     
                     }
                     else
                     {
				...
				case 102: //Edycja danych firmy
				     if(TheSpeditor->id == 0)
				     {
				     	FMessage("Wybierz pracownika aby zmieniæ dane jego pracodawcy");
					 }
					 else
					 {
					 	new_add_s = 1;
				...
				case 103: //Edycja danych kierowcy
 					 if(TheSpeditor->id == 0)
				     {
				     	FMessage("Wybierz pracownika aby zmieniæ jego dane");
					 }
					 else
					 {
					 	new_add_d = 1;
